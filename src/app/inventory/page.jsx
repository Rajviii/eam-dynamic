...
export default function InventoryPage() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const url = modalMode === 'add' ? '/api/inventory' : `/api/inventory/${formData.id}`;
  ...
  if (loading && inventory.length === 0) {
    return <div className="p-8 text-center text-slate-500 animate-pulse">Loading inventory...</div>;
  }

  return (
    <div className="space-y-6 relative">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold text-slate-900 dark:text-slate-50">Inventory & Parts</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white dark:bg-slate-900 p-5 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
          ...
          <div className="text-2xl font-bold text-slate-900 dark:text-slate-50 mt-2">{total}</div>
        </div>
        <div className="bg-white dark:bg-slate-900 p-5 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
          <h3 className="text-slate-500 dark:text-slate-400 text-sm font-medium">Low Stock Alerts</h3>
          <div className="text-2xl font-bold text-orange-600 dark:text-orange-500 mt-2">
            {inventory.filter(i => i.status === 'Low Stock').length}
          </div>
        </div>
        <div className="bg-white dark:bg-slate-900 p-5 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
          <h3 className="text-slate-500 dark:text-slate-400 text-sm font-medium">Out of Stock</h3>
          <div className="text-2xl font-bold text-red-600 dark:text-red-500 mt-2">
            {inventory.filter(i => i.status === 'Out of Stock').length}
          </div>
        </div>
        <div className="bg-white dark:bg-slate-900 p-5 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
          <h3 className="text-slate-500 dark:text-slate-400 text-sm font-medium">Inventory Value</h3>
          <div className="text-2xl font-bold text-slate-900 dark:text-slate-50 mt-2">
            ${inventory.reduce((acc, part) => acc + (part.onHand * part.unitCost), 0).toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}
          </div>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left text-slate-600 dark:text-slate-400">
            <thead className="text-xs text-slate-700 uppercase bg-slate-50 dark:bg-slate-800 dark:text-slate-300 border-b border-slate-200 dark:border-slate-700">
              <tr>
                <SortableHeader label="Part Number" columnKey="partNumber" sortBy={sortBy} sortOrder={sortOrder} handleSort={handleSort} />
                ...
                <SortableHeader label="On Hand" columnKey="onHand" sortBy={sortBy} sortOrder={sortOrder} handleSort={handleSort} className="text-right" />
                <SortableHeader label="Unit Cost" columnKey="unitCost" sortBy={sortBy} sortOrder={sortOrder} handleSort={handleSort} className="text-right" />
            ...
            <tbody>
              {inventory.map((item, index) => (
                <tr key={item.id} className={`border-b dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors ${index === inventory.length - 1 ? 'border-b-0' : ''}`}>
                  <td className="px-6 py-4 font-medium text-blue-600 dark:text-blue-400">{item.partNumber}</td>
                  <td className="px-6 py-4 text-slate-900 dark:text-slate-100 font-medium">{item.description}</td>
              ...
              {inventory.length === 0 && !loading && (
                <tr>
                  <td colSpan="7" className="px-6 py-8 text-center text-slate-500">No parts found. Click 'Add Part' to create one.</td>
                </tr>
              )}
            </tbody>
          </table>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xl rounded-xl w-full max-w-md overflow-hidden transform transition-all">
            <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-slate-50 dark:bg-slate-800/50">
              <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-50">
                {modalMode === 'add' ? 'Add New Part' : 'Edit Part'}
              </h2>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="pt-4 flex items-center justify-end gap-3">
                ...
                <button type="submit" className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors">
                  {modalMode === 'add' ? 'Create Part' : 'Save Changes'}
                </button>
              </div>
            </form>
          </div>
