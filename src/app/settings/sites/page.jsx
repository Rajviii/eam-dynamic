...
export default function SitesPage() {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`/api/settings/sites/${id}`, { method: 'DELETE' });
      if (res.ok) refresh();
    } catch (err) {
      console.error(err);
    }
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const url = modalMode === 'add' ? '/api/settings/sites' : `/api/settings/sites/${formData.id}`;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-2xl font-semibold text-slate-900 dark:text-slate-50">Sites & Locations</h1>
        </div>
          ...
          <table className="w-full text-sm text-left text-slate-600 dark:text-slate-400">
            <thead className="text-xs text-slate-700 uppercase bg-slate-50 dark:bg-slate-800 dark:text-slate-300 border-b border-slate-200 dark:border-slate-700">
              <tr>
                ...
                <SortableHeader label="Location" columnKey="location" sortBy={sortBy} sortOrder={sortOrder} handleSort={handleSort} />
            ...
            <tbody>
              ...
              {data.length === 0 && !loading && (
                <tr><td colSpan="3" className="px-6 py-8 text-center text-slate-500">No sites found.</td></tr>
              )}
              {loading && (
                <tr><td colSpan="3" className="px-6 py-8 text-center text-slate-500 animate-pulse">Loading sites...</td></tr>
              )}
            </tbody>
          </table>
