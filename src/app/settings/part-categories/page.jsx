...
export default function PartCategoriesPage() {
  const { data, total, loading, page, limit, setPage, handleSort, handleSearch, refresh, sortBy, sortOrder } = useDataTable({ endpoint: '/api/settings/part-categories', initialSortBy: 'name' });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`/api/settings/part-categories/${id}`, { method: 'DELETE' });
      if (res.ok) refresh();
    } catch (err) {
      console.error(err);
    }
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const url = modalMode === 'add' ? '/api/settings/part-categories' : `/api/settings/part-categories/${formData.id}`;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-2xl font-semibold text-slate-900 dark:text-slate-50">Part Categories</h1>
        </div>
